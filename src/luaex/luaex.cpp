...
void Context::dispose() {
	...
	auto iter = std::remove_if(Context::values.begin(), Context::values.end(), [self](std::shared_ptr<Context> ctx) -> bool {
		return self == ctx;
	});
...
void Context::put(const std::string & name, linb::any var) {
	map[name] = var;
}
linb::any Context::get(const std::string & name)  const {
	return map.at(name);
}
...
Error Interpreter::call(const std::string& function, std::vector<Object> args, std::vector<Type> returnType, std::vector<Variant>& returnOut) {
...
void Interpreter::setGlobalString(const std::string & name, const std::string & value) {
...
Error Interpreter::getGlobalString(const std::string & name, std::string& outStr) {
	int type = lua_getglobal(state, name.c_str());
	if (type != LUA_TSTRING) {
...
std::unordered_map<std::string, Object> Interpreter::getAllVariables() const {
	...
	while (lua_next(state, -2) != 0) {
		...
		if (type == LUA_TNUMBER) {
			ret.insert_or_assign(key, create(lua_tonumber(state, -1)));
		} else if (type == LUA_TBOOLEAN) {
			ret.insert_or_assign(key, create(lua_toboolean(state, -1) == 1));
...
bool Interpreter::isOccurred() const {
