...
namespace planet {
TextureInfoCollection::TextureInfoCollection(const std::string & baseDirectory) : baseDirectory(baseDirectory), textureInfoVec() {
}
...
std::string TextureInfoCollection::serialize() const {
	...
	for (auto texInfo : textureInfoVec) {
		...
		textureO["baseFileName"] = picojson::value(texInfo.baseFileName);
		textureO["reference"] = picojson::value(texInfo.reference);
		if (texInfo.mappingRule.all) {
			mappingRuleO["all"] = picojson::value(*texInfo.mappingRule.all);
		}
		// front, back
		if (texInfo.mappingRule.front) {
			mappingRuleO["front"] = picojson::value(*texInfo.mappingRule.front);
		}
		if (texInfo.mappingRule.back) {
			mappingRuleO["back"] = picojson::value(*texInfo.mappingRule.back);
		}
		// left, right
		if (texInfo.mappingRule.left) {
			mappingRuleO["left"] = picojson::value(*texInfo.mappingRule.left);
		}
		if (texInfo.mappingRule.right) {
			mappingRuleO["right"] = picojson::value(*texInfo.mappingRule.right);
		}
		// top, bottom
		if (texInfo.mappingRule.top) {
			mappingRuleO["top"] = picojson::value(*texInfo.mappingRule.top);
		}
		if (texInfo.mappingRule.bottom) {
			mappingRuleO["bottom"] = picojson::value(*texInfo.mappingRule.bottom);
		}
...
mapbox::util::optional<TextureInfo> TextureInfoCollection::getTextureInfo(const std::string & reference) const {
