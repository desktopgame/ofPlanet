...
ScriptBiome::ScriptBiome(const std::string& filename) :lua(), ctx(nullptr), table(nullptr) {
	...
	for (int x = 0; x < this->table->getXSize(); x++) {
		for (int y = 0; y < this->table->getYSize(); y++) {
			for (int z = 0; z < this->table->getZSize(); z++) {
				blockTable.set(x, y, z, this->table->get(x, y, z));
			}
		}
	}
}
...
float ScriptBiome::onFixHeight(float y) {
	...
	lua.call("onFixHeight", std::vector<Object>{ create(y) }, std::vector<Type>{ T_NUMBER }, r);
	return static_cast<float>(r.at(0).number);
}

void ScriptBiome::onGenerateTerrain(BlockTable & blockTable, int x, int y, int z) {
	std::vector<Variant> r;
...
void ScriptBiome::onGenerateStructures(BlockTable & blockTable) {
	std::vector<Variant> r;
	lua.call("onGenerateStructures",
		std::vector<Object>{ },
		std::vector<Type>{ }, r);
}

void ScriptBiome::onGenerateCave(BlockTable & blockTable, int x, int y, int z, float noise) {
	std::vector<Variant> r;
	lua.call("onGenerateCave",
		std::vector<Object>{ create(x), create(y), create(z), create(static_cast<double>(noise)) },
		std::vector<Type>{ }, r);
}
...
int lua_getblock(lua_State * state) {
	...
	lua_pushstring(state, blockpack->getBlock(table->get(x, y, z).id)->getName().c_str());
	return 1;
}
