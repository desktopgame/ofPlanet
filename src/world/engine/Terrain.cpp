...
namespace planet {
Terrain::Terrain() : cells(), pockets(), vmin(32768,32768,32768),vmax(-32768,-32768,-32768) {
}
...
glm::ivec3 Terrain::getMin() const {
	return vmin;
}
glm::ivec3 Terrain::getMax() const {
	return vmax;
}
...
Cell Terrain::findCell(int x, int z) const {
	auto iter = std::find_if(cells.begin(), cells.end(), [x, z](const Cell& e) -> bool {
		return e.x == x && e.z == z;
	});
	return *iter;
}
