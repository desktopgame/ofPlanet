...
std::shared_ptr<BlockPack> BlockPack::load(const BlockInfoCollection & blockInfoCollection) {
	...
	for (int i = 0; i < blockInfoCollection.getBlockInfoCount(); i++) {
		...
		auto block = std::make_shared<Block>(info.reference, info.textue, i);
		ret->addBlock(block);
	}
...
bool BlockPack::isSelected() const {
...
std::shared_ptr<BlockPack> BlockPack::registerBlockPack(const std::shared_ptr<BlockPack> pack) {
