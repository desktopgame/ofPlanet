...
class BlockPack {
public:
	static std::shared_ptr<BlockPack> load(const BlockInfoCollection& blockInfoCollection);
...
protected:
	BlockPack();
	static std::shared_ptr<BlockPack> registerBlockPack(const std::shared_ptr<BlockPack> pack);
private:
	bool selected;
	std::string name;
