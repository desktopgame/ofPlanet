...
//
//
// Copyright (c) 2016 Denilson das Merc棚s Amorim
//
...
class any final {
        ...
        void swap(any& rhs) noexcept {
                if (this->vtable != rhs.vtable) {
                        ...
                        if (this->vtable != nullptr) {
                                ...
                                // this->vtable = nullptr; -- unneeded, see
                                // below
                        }
       ...
       private:  // Storage and Virtual Method Table
        ...
        struct vtable_type {
                ...
                /// The state of the union after this call is unspecified,
                /// caller must ensure not to use src anymore.
                void (*destroy)(storage_union&) noexcept;

                /// Copies the **inner** content of the src union into the yet
                /// unitialized dest union. As such, both inner objects will
                /// have the same state, but on separate memory locations.
                void (*copy)(const storage_union& src, storage_union& dest);

                /// Moves the storage from src to the yet unitialized dest
                /// union. The state of src after this call is unspecified,
                /// caller must ensure not to use src anymore.
        ...
        struct vtable_dynamic {
                ...
                static void copy(const storage_union& src,
                                 storage_union& dest) {
                        dest.dynamic =
                            new T(*reinterpret_cast<const T*>(src.dynamic));
                }

                static void move(storage_union& src,
                                 storage_union& dest) noexcept {
                        dest.dynamic = src.dynamic;
                        src.dynamic = nullptr;
                }

                static void swap(storage_union& lhs,
                                 storage_union& rhs) noexcept {
        ...
        struct vtable_stack {
                ...
                static void copy(const storage_union& src,
                                 storage_union& dest) {
                        new (&dest.stack)
                            T(reinterpret_cast<const T&>(src.stack));
                }

                static void move(storage_union& src,
                                 storage_union& dest) noexcept {
                        // one of the conditions for using vtable_stack is a
                        // nothrow move constructor, so this move constructor
                        // will never throw a exception.
                        new (&dest.stack)
                            T(std::move(reinterpret_cast<T&>(src.stack)));
                        destroy(src);
                }
...
inline ValueType any_cast(any&& operand) {
        using can_move = std::integral_constant<
            bool, std::is_move_constructible<ValueType>::value &&
