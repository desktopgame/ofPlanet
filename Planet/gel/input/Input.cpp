...
ButtonState Input::getMouseState(MouseButton button) {
	...
	case gel::MouseButton::Left:
		status = glfwGetMouseButton(wnd, GLFW_MOUSE_BUTTON_LEFT);
		break;
	case gel::MouseButton::Right:
		status = glfwGetMouseButton(wnd, GLFW_MOUSE_BUTTON_RIGHT);
		break;
	case gel::MouseButton::Middle:
		status = glfwGetMouseButton(wnd, GLFW_MOUSE_BUTTON_MIDDLE);
		break;
	default:
		break;
	}
