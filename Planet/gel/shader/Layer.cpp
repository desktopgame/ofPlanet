...
Layer::Layer(const std::shared_ptr<Shader>& shader, const NameRule nameRule)
    : shader(shader), screenBuffer(shader, nameRule), initFlag(false) {}
...
std::shared_ptr<Shader> Layer::getShader() const { return shader; }
void Layer::batchLayer() {}
DelegateLayer::DelegateLayer(const std::shared_ptr<Shader>& shader,
                             const NameRule nameRule, Action action)
    : Layer(shader, nameRule), action(action) {}
void DelegateLayer::batchLayer() { action(); }
}  // namespace gel
