...
enum class PixelMatch { EqualRGB, EqualRGBA };
enum class PixelReplace { ReplaceRGB, ReplaceRGBA };
class PixelBuffer {
       public:
        ...
        int getWidth() const;
        int getHeight() const;
        static void replace(GLubyte* source, int width, int height,
                            PixelMatch match, PixelReplace replace,
                            Pixel oldPix, Pixel newPix);
