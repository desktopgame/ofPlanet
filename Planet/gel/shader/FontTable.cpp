...
namespace gel {
	...
	void FontTable::init(const std::vector<std::string> lines, Rule rule)
	{
		...
		for (std::string line : lines) {
			for (char c : line) {
				auto tex = gel::AssetDatabase::getAsset<ITexture>(rule(c, row, col));
				gel::Sprite sprite(gel::ShaderRegistry::getInstance().get("Texture2D"), gel::NameRule());
				sprite.init(tex->getID(), glm::vec2(0, 0), tex->getSize(), 1.0f);
	...
	void FontTable::draw(std::shared_ptr<Camera> camera, glm::vec2 pos, glm::vec2 interval, const std::string str)
	{
		glm::vec2 npos = pos;
		for (char c : str) {
			draw(camera, npos, c);
