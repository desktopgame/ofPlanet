...
ImGuiStyle::ImGuiStyle() {
        ...
        WindowRounding = 7.0f;  // Radius of window corners rounding. Set to
        ...
        WindowMenuButtonPosition =
            ImGuiDir_Left;  // Position of the collapsing/docking button in the
                            // title bar (left/right). Defaults to
                            // ImGuiDir_Left.
        ChildRounding = 0.0f;  // Radius of child window corners rounding. Set
        ...
        PopupRounding = 0.0f;    // Radius of popup window corners rounding. Set
                                 // to 0.0f to have rectangular child windows
        ...
        ColumnsMinSpacing =
            6.0f;  // Minimum horizontal spacing between two columns. Preferably
                   // > (FramePadding.x + 1).
        ScrollbarSize = 16.0f;  // Width of the vertical scrollbar, Height of
                                // the horizontal scrollbar
        ScrollbarRounding =
            9.0f;  // Radius of grab corners rounding for scrollbar
        GrabMinSize =
            10.0f;  // Minimum width/height of a grab box for slider/scrollbar
        GrabRounding = 0.0f;  // Radius of grabs corners rounding. Set to 0.0f
        ...
        SelectableTextAlign =
            ImVec2(0.0f, 0.0f);  // Alignment of selectable text when button is
                                 // larger than text.
        DisplayWindowPadding =
            ImVec2(19, 19);  // Window position are clamped to be visible within
                             // the display area by at least this amount. Only
        ...
        AntiAliasedLines = true;  // Enable anti-aliasing on lines/borders.
                                  // Disable if you are really short on CPU/GPU.
        AntiAliasedFill = true;   // Enable anti-aliasing on filled shapes
...
FILE* ImFileOpen(const char* filename, const char* mode) {
        ...
        // UTF-8 filenames. Converting both strings from UTF-8 to wchar format
        // (using a single allocation, because we can)
...
int ImTextStrFromUtf8(ImWchar* buf, int buf_size, const char* in_text,
        ...
        while (buf_out < buf_end - 1 &&
               (!in_text_end || in_text < in_text_end) && *in_text) {
                ...
                if (c == 0) break;
                if (c < 0x10000)  // FIXME: Losing characters that don't fit in
                                  // 2 bytes
                        *buf_out++ = (ImWchar)c;
        }
        *buf_out = 0;
...
bool ImGuiTextFilter::PassFilter(const char* text, const char* text_end) const {
        ...
        for (int i = 0; i != Filters.Size; i++) {
                ...
                if (f.b[0] == '-') {
                        // Subtract
                        if (ImStristr(text, text_end, f.begin() + 1, f.end()) !=
                            NULL)
                                return false;
                } else {
                        // Grep
                        if (ImStristr(text, text_end, f.begin(), f.end()) !=
                            NULL)
                                return true;
                }
        }
            ...
            (line_height -
             GImGui->Style.ItemSpacing
                 .y);  // If we end up needing more accurate data (to e.g. use
                       // SameLine) we may as well make the clipper have a
                       // fourth step to let user process and display the last
                       // item in their list.
...
bool ImGuiListClipper::Step() {
        ...
        if (StepNo == 0)  // Step 0: the clipper let you process the first
                          // element, regardless of it being visible or not, so
        ...
        if (StepNo ==
            1)  // Step 1: the clipper infer height from first element,
                ...
                // position the cursor before the first element.
        {
                ...
                IM_ASSERT(items_height >
                          0.0f);  // If this triggers, it means Item 0 hasn't
        ...
        if (StepNo == 3)  // Step 3: the clipper validate that we have reached
                          // the expected Y position (corresponding to element
                          // DisplayEnd), advance the cursor to the end of the
...
void ImGui::RenderTextClippedEx(ImDrawList* draw_list, const ImVec2& pos_min,
        ...
        if (align.x > 0.0f)
                pos.x = ImMax(
                    pos.x, pos.x + (pos_max.x - pos.x - text_size.x) * align.x);
        if (align.y > 0.0f)
                pos.y = ImMax(
                    pos.y, pos.y + (pos_max.y - pos.y - text_size.y) * align.y);
...
void ImGui::SetFocusID(ImGuiID id, ImGuiWindow* window) {
        ...
        // Assume that SetFocusID() is called in the context where its NavLayer
        // is the current layer, which is the case everywhere we call it.
                ...
                g.FocusRequestNextCounterTab =
                    ...
                    (g.IO.KeyShift ? (is_tab_stop ? -1 : 0)
                                   : +1);  // Modulo on index will be applied at
                                           // the end of frame once we've got
...
void ImGui::SetClipboardText(const char* text) {
        if (GImGui->IO.SetClipboardTextFn)
                GImGui->IO.SetClipboardTextFn(GImGui->IO.ClipboardUserData,
                                              text);
}
...
static ImDrawList* GetForegroundDrawList(ImGuiWindow*) {
        // This seemingly unnecessary wrapper simplifies compatibility between
        // the 'master' and 'docking' branches.
        ...
        if (g.IO.MouseClicked[0]) {
                if (g.HoveredRootWindow != NULL) {
                        ...
                        if (g.IO.ConfigWindowsMoveFromTitleBarOnly &&
                            !(g.HoveredRootWindow->Flags &
                              ImGuiWindowFlags_NoTitleBar))
                                if (!g.HoveredRootWindow->TitleBarRect()
                                         .Contains(g.IO.MouseClickedPos[0]))
                                        g.MovingWindow = NULL;
...
static void ImGui::UpdateMouseInputs() {
        ...
        for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
                        ...
                        if ((float)(g.Time - g.IO.MouseClickedTime[i]) <
                            g.IO.MouseDoubleClickTime) {
                                ...
                                g.IO.MouseClickedTime[i] =
                                    -FLT_MAX;  // so the third click isn't
                ...
                if (g.IO.MouseClicked[i])  // Clicking any mouse button
                                           // reactivate mouse hovering which
                                           // may have been deactivated by
...
void ImGui::UpdateMouseWheel() {
        ...
        // ImGuiWindowFlags_NoInputs or ImGuiWindowFlags_NoScrollbar are also
        // set).
        ...
        if (scroll_allowed &&
            ...
            !g.IO.KeyCtrl) {
                ImVec2 max_step = (window->ContentsRegionRect.GetSize() +
                                   window->WindowPadding * 2.0f) *
                                  0.67f;

                // Vertical Mouse Wheel Scrolling (hold Shift to scroll
                // horizontally)
                if (g.IO.MouseWheel != 0.0f && !g.IO.KeyShift) {
                        float scroll_step = ImFloor(
                            ImMin(5 * window->CalcFontSize(), max_step.y));
                        SetWindowScrollY(
                            window,
                            window->Scroll.y - g.IO.MouseWheel * scroll_step);
                } else if (g.IO.MouseWheel != 0.0f && g.IO.KeyShift) {
                        float scroll_step = ImFloor(
                            ImMin(2 * window->CalcFontSize(), max_step.x));
                        SetWindowScrollX(
                            window,
                            window->Scroll.x - g.IO.MouseWheel * scroll_step);
                }

                // Horizontal Mouse Wheel Scrolling (for hardware that supports
                // it)
...
void ImGui::NewFrame() {
        ...
        IM_ASSERT(g.Style.Alpha >= 0.0f && g.Style.Alpha <= 1.0f &&
                  "Invalid style setting. Alpha cannot be negative (allows us "
                  "to avoid a few clamps in color computations)!");
        ...
        // Perform simple check: required key mapping (we intentionally do NOT
        // check all keys to not pressure user into setting up everything, but
        // Space is required and was only recently added in 1.60 WIP)
        ...
        g.PlatformImePos =
            ImVec2(1.0f, 1.0f);  // OS Input Method Editor showing on top-left
                                 // of our window by default
                ...
                if (g.FocusRequestNextCounterAll != INT_MAX &&
                    window->DC.FocusCounterAll != -1)
                        g.FocusRequestCurrCounterAll =
                            ImModPositive(g.FocusRequestNextCounterAll,
                                          window->DC.FocusCounterAll + 1);
                if (g.FocusRequestNextCounterTab != INT_MAX &&
                    window->DC.FocusCounterTab != -1)
                        g.FocusRequestCurrCounterTab =
                            ImModPositive(g.FocusRequestNextCounterTab,
                                          window->DC.FocusCounterTab + 1);
                g.FocusRequestNextWindow = NULL;
                g.FocusRequestNextCounterAll = g.FocusRequestNextCounterTab =
                    INT_MAX;
        }
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list,
        ...
        // because ImDrawList let all your vertices pass. You can use the
        // Metrics window to inspect draw list contents. B) If you need/want
        // meshes with more than 64K vertices, uncomment the '#define ImDrawIdx
        // unsigned int' line in imconfig.h to set the index size to 4 bytes.
        //    You'll need to handle the 4-bytes indices to your renderer. For
        //    example, the OpenGL example code detect index size at compile-time
        //    by doing:
        ...
        //    generally supported by most API.
        // C) If for some reason you cannot use 4 bytes indices or don't want
        // to, a workaround is to call BeginChild()/EndChild() before reaching
...
void ImGui::EndFrame() {
        ...
        if (g.CurrentWindowStack.Size != 1) {
                if (g.CurrentWindowStack.Size > 1) {
                        ...
                        while (g.CurrentWindowStack.Size >
                               1)  // FIXME-ERRORHANDLING
                                End();
                } else {
...
void ImGui::Render() {
                ...
                if (windows_to_render_front_most[n] &&
                    IsWindowActiveAndVisible(
                        ...
                        AddRootWindowToDrawData(
                            windows_to_render_front_most[n]);
        ...
        // (Legacy) Call the Render callback function. The current prefer way is
        // to let the user retrieve GetDrawData() and call the render function
        // themselves.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
static void FindHoveredWindow() {
        ...
        for (int i = g.Windows.Size - 1; i >= 0; i--) {
                ...
                // Those seemingly unnecessary extra tests are because the code
                // here is a little different in viewport/docking branches.
...
bool ImGui::IsMouseClicked(int button, bool repeat) {
        ...
        if (repeat && t > g.IO.KeyRepeatDelay) {
                ...
                int amount = CalcTypematicPressedRepeatAmount(
                    t, t - g.IO.DeltaTime, g.IO.KeyRepeatDelay,
...
void ImGui::EndChild() {
        ...
        if (window->BeginCount > 1) {
                ...
                if (window->AutoFitChildAxises &
                    (1 << ImGuiAxis_X))  // Arbitrary minimum zero-ish child
...
static void CheckStacksSize(ImGuiWindow* window, bool write) {
        ...
        // DC.TextWrapPos (per window) to allow user to conveniently push once
        ...
        }  // Too few or too many EndGroup()
        {
                ...
                if (write)
                        *p_backup = (short)current;
                        ...
                        IM_ASSERT(*p_backup == current &&
                                  "BeginMenu/EndMenu or BeginPopup/EndPopup "
                                  "Mismatch");
                p_backup++;
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size,
        ...
        if (!(flags & ImGuiWindowFlags_NoSavedSettings))
                if (ImGuiWindowSettings* settings =
                        ...
                        window->SettingsIdx =
                            g.SettingsWindows.index_from_ptr(settings);
...
static void CalcResizePosSizeFromAnyCorner(ImGuiWindow* window,
        ...
        ImVec2 size_constrained =
            CalcSizeAfterConstraint(window, size_expected);
        *out_pos = pos_min;
...
static void ImGui::UpdateManualResize(ImGuiWindow* window,
        ...
        const float grip_hover_inner_size =
            (float)(int)(grip_draw_size * 0.75f);
        ...
        for (int border_n = 0; border_n < resize_border_count; border_n++) {
                ...
                ImRect border_rect = GetResizeBorderRect(
                    ...
                    WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS);
                ButtonBehavior(
                    border_rect, window->GetID((void*)(intptr_t)(border_n + 4)),
...
static inline void ClampWindowRect(ImGuiWindow* window, const ImRect& rect,
        ...
        window->Pos =
            ImMin(rect.Max - padding,
                  ImMax(window->Pos + size_for_clamping, rect.Min + padding) -
                      size_for_clamping);
}
...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow* window) {
        ...
        if (border_held != -1) {
                ...
                static const ImGuiResizeBorderDef resize_border_def[4] = {
                    {ImVec2(0, +1), ImVec2(0, 0), ImVec2(1, 0),
                     IM_PI * 1.50f},  // Top
                    {ImVec2(-1, 0), ImVec2(1, 0), ImVec2(1, 1),
                     IM_PI * 0.00f},  // Right
                    {ImVec2(0, -1), ImVec2(1, 1), ImVec2(0, 1),
                     IM_PI * 0.50f},  // Bottom
                    {ImVec2(+1, 0), ImVec2(0, 1), ImVec2(0, 0),
                     IM_PI * 1.00f}  // Left
                };
        ...
        if (has_collapse_button)
                if (CollapseButton(window->GetID("#COLLAPSE"),
                        ...
                        window->WantCollapseToggle =
                            true;  // Defer actual collapsing to next frame as
                                   // we are too far in the Begin() function
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags) {
        ...
        if (window_just_created) {
                ImVec2 size_on_first_use =
                    (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSize)
                        ...
                        : ImVec2(0.0f,
                                 0.0f);  // Any condition flag will do since we
        ...
        if (flags & ImGuiWindowFlags_Popup) {
                ...
                window_just_activated_by_user |=
                    (window->PopupId !=
                     popup_ref.PopupId);  // We recycle popups so treat window
                                          // as activated if popup id changed
        ...
        if (first_begin_of_the_frame) {
                ...
                // Size/SizeContents for reappearing popups/tooltips early in
                ...
                window->SizeFull =
                    CalcSizeAfterConstraint(window, window->SizeFull);
                window->Size =
                ...
                if (!window->Collapsed) {
                        ...
                        // after size constraints have been applied
                        float size_x_for_scrollbars =
                            size_full_modified.x != FLT_MAX
                                ? window->SizeFull.x
                                : window->SizeFullAtLastBegin.x;
                        float size_y_for_scrollbars =
                            size_full_modified.y != FLT_MAX
                                ? window->SizeFull.y
                ...
                if (!window_pos_set_by_api &&
                    ...
                    window->AutoFitFramesY <= 0) {
                        if (g.IO.DisplaySize.x > 0.0f &&
                            g.IO.DisplaySize.y >
                                0.0f)  // Ignore zero-sized display explicitly
                                       // to avoid losing positions if a window
                                       // manager reports zero-sized window when
                                       // initializing or minimizing.
                        {
                                ImVec2 clamp_padding =
                                    ImMax(style.DisplayWindowPadding,
                                          style.DisplaySafeAreaPadding);
                                ClampWindowRect(window, viewport_rect,
                                                clamp_padding);
                        }
                }
                ...
                // Apply scrolling
                window->Scroll =
                    CalcNextScrollFromScrollTargetAndClamp(window, true);
                ...
                if (window->Size.x > 0.0f &&
                    ...
                    !(flags & ImGuiWindowFlags_AlwaysAutoResize))
                        ...
                        window->ItemWidthDefault =
                            (float)(int)(g.FontSize * 16.0f);

                // Store a backup of SizeFull which we will use next frame to
                // decide if we need scrollbars.
                window->SizeFullAtLastBegin = window->SizeFull;

                // UPDATE RECTANGLES

                // Update various regions. Variables they depends on are set
                // above in this function.
                // FIXME: window->ContentsRegionRect.Max is currently very
                // misleading / partly faulty, but some BeginChild() patterns
                // relies on it. NB: WindowBorderSize is included in
                // WindowPadding _and_ ScrollbarSizes so we need to cancel one
                // out.
                ...
                window->ContentsRegionRect.Max.y =
                    ...
                    (window->SizeContentsExplicit.y != 0.0f
                         ...
                         : (window->Size.y - window->ScrollbarSizes.y +
                            ImMin(window->ScrollbarSizes.y,
                                  window->WindowBorderSize)));

                // Inner rectangle
                // We set this up after processing the resize grip so that our
                // clip rectangle doesn't lag by a frame Note that if our window
                // is collapsed we will end up with an inverted (~null) clipping
                // rectangle which is the correct behavior.
                const ImRect title_bar_rect = window->TitleBarRect();
                window->InnerVisibleRect.Min.x =
                    title_bar_rect.Min.x + window->WindowBorderSize;
                window->InnerVisibleRect.Min.y =
                    title_bar_rect.Max.y + window->MenuBarHeight() +
                    (((flags & ImGuiWindowFlags_MenuBar) ||
                      !(flags & ImGuiWindowFlags_NoTitleBar))
                         ? style.FrameBorderSize
                         : window->WindowBorderSize);
                window->InnerVisibleRect.Max.x =
                    window->Pos.x + window->Size.x -
                    ImMax(window->ScrollbarSizes.x, window->WindowBorderSize);
                window->InnerVisibleRect.Max.y =
                    window->Pos.y + window->Size.y -
                    ImMax(window->ScrollbarSizes.y, window->WindowBorderSize);

                // Outer host rectangle for drawing background and borders
                ImRect host_rect = ((flags & ImGuiWindowFlags_ChildWindow) &&
                                    !(flags & ImGuiWindowFlags_Popup) &&
                ...
                // Inner work/clipping rectangle will extend a little bit
                // outside the work region. This is to allow e.g. Selectable or
                // CollapsingHeader or some separators to cover that space.
                // Force round operator last to ensure that e.g.
                // (int)(max.x-min.x) in user's render code produce correct
                // result.
                window->InnerWorkRect.Min.x =
                    ImFloor(0.5f + window->InnerVisibleRect.Min.x +
                            ImMax(0.0f, ImFloor(window->WindowPadding.x * 0.5f -
                                                window->WindowBorderSize)));
                window->InnerWorkRect.Min.y =
                    ImFloor(0.5f + window->InnerVisibleRect.Min.y);
                window->InnerWorkRect.Max.x =
                    ImFloor(0.5f + window->InnerVisibleRect.Max.x -
                            ImMax(0.0f, ImFloor(window->WindowPadding.x * 0.5f -
                                                window->WindowBorderSize)));
                window->InnerWorkRect.Max.y =
                    ImFloor(0.5f + window->InnerVisibleRect.Max.y);
                ...
                window->DrawList->Flags =
                    (g.Style.AntiAliasedLines ? ImDrawListFlags_AntiAliasedLines
                                              : 0) |
                    (g.Style.AntiAliasedFill ? ImDrawListFlags_AntiAliasedFill
                                             : 0);
                ...
                if (dim_bg_for_window_list &&
                    window == g.NavWindowingTargetAnim) {
                        ...
                        if (!bb.Contains(
                                viewport_rect))  // Avoid drawing if the window
                                                 // covers all the viewport
                                                 // anyway
                                window->DrawList->AddRectFilled(
                                    bb.Min, bb.Max,
                                    GetColorU32(
                ...
                window->DC.ItemFlags = parent_window
                                           ? parent_window->DC.ItemFlags
                ...
                if ((flags & ImGuiWindowFlags_ChildWindow) &&
                    (window->DC.ItemFlags != parent_window->DC.ItemFlags)) {
                        window->DC.ItemFlags = parent_window->DC.ItemFlags;
                        window->DC.ItemFlagsStack.push_back(
                            window->DC.ItemFlags);
                }
        ...
        if (flags & ImGuiWindowFlags_ChildWindow) {
                ...
                // windows. Mark them as collapsed so commands are skipped
                // earlier (we can't manually collapse them because they have no
                // title bar).
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_first_use,
        ...
        // Old API feature: we could pass the initial window size as a
        // parameter. This was misleading because it only had an effect if the
        // window didn't have data in the .ini file.
        if (size_first_use.x != 0.0f || size_first_use.y != 0.0f)
                SetNextWindowSize(size_first_use, ImGuiCond_FirstUseEver);

        // Old API feature: override the window background alpha with a
        // parameter.
...
void ImGui::SetCurrentFont(ImFont* font) {
        ...
        g.Font = font;
        g.FontBaseSize = ImMax(
            1.0f, g.IO.FontGlobalScale * g.Font->FontSize * g.Font->Scale);
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, const ImVec2& val) {
        ...
        IM_ASSERT(0 &&
                  "Called PushStyleVar() ImVec2 variant but variable is not a "
                  "ImVec2!");
}
                ...
                switch (flags & (ImGuiHoveredFlags_RootWindow |
                        ...
                        case ImGuiHoveredFlags_ChildWindows:
                                if (g.HoveredWindow == NULL ||
                                    !IsWindowChildOf(g.HoveredWindow,
                                                     g.CurrentWindow))
                                        return false;
                                break;
                        default:
...
bool ImGui::IsWindowFocused(ImGuiFocusedFlags flags) {
        ...
        switch (flags & (ImGuiFocusedFlags_RootWindow |
                ...
                case ImGuiFocusedFlags_RootWindow |
                    ImGuiFocusedFlags_ChildWindows:
                        return g.NavWindow && g.NavWindow->RootWindow ==
                                                  g.CurrentWindow->RootWindow;
                case ImGuiFocusedFlags_RootWindow:
...
void ImGui::SetWindowScrollX(ImGuiWindow* window, float new_scroll_x) {
        window->DC.CursorMaxPos.x +=
            window->Scroll
                .x;  // SizeContents is generally computed based on CursorMaxPos
                     // which is affected by scroll position, so we need to
                     // apply our change to it.
...
void ImGui::SetWindowScrollY(ImGuiWindow* window, float new_scroll_y) {
        window->DC.CursorMaxPos.y +=
            window->Scroll
                .y;  // SizeContents is generally computed based on CursorMaxPos
                     // which is affected by scroll position, so we need to
                     // apply our change to it.
...
void ImGui::SetNextWindowContentSize(const ImVec2& size) {
        ...
        g.NextWindowData.ContentSizeVal =
            size;  // In Begin() we will add the size of window decorations
                   // (title bar, menu etc.) to that to form a SizeContents
                   // value.
}
...
void ImGui::SetScrollY(float scroll_y) {
        ...
        window->ScrollTarget.y =
            ...
            window->MenuBarHeight();  // title bar height canceled out when
...
void ImGui::EndGroup() {
        ...
        window->DC.CurrLineTextBaseOffset = ImMax(
            ...
            group_data
                .BackupCurrLineTextBaseOffset);  // FIXME: Incorrect, we should
                                                 // grab the base offset from
                                                 // the *first line* of the
                                                 // group but it is hard to
                                                 // obtain now.
        ...
        // used in that context. (The tests not symmetrical because
            ...
            IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup)) {
                ImGuiID id =
                    str_id ? window->GetID(str_id)
                           : window->DC
                                 .LastItemId;  // If user hasn't passed an ID,
                                               // we can use the LastItemID.
                                               // Using LastItemID as a Popup ID
                                               // won't conflict!
                IM_ASSERT(id !=
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window,
        ...
        // When popups are stacked, clicking on a lower level popups puts focus
        // back to it and close popups above it. Don't close our own child popup
        // windows.
        ...
        if (ref_window) {
                ...
                for (; popup_count_to_keep < g.OpenPopupStack.Size;
                     popup_count_to_keep++) {
                        ...
                        // Trim the stack when popups are not direct descendant
                        // of the reference window (the reference window is
                        // often the NavWindow)
                        bool popup_or_descendent_is_ref_window = false;
                        for (int m = popup_count_to_keep;
                             m < g.OpenPopupStack.Size &&
                             !popup_or_descendent_is_ref_window;
                             m++)
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags) {
        ...
        if (!is_open)  // NB: Begin can return false when the popup is
...
void ImGui::EndPopup() {
        ...
        IM_ASSERT(g.CurrentWindow->Flags &
                  ImGuiWindowFlags_Popup);  // Mismatched
                                            // BeginPopup()/EndPopup() calls
        ...
        if (window->Flags & ImGuiWindowFlags_ChildMenu) {
                ...
                float horizontal_overlap =
                    g.Style.ItemInnerSpacing
                        .x;  // We want some overlap to convey the relative
        ...
        if (window->Flags & ImGuiWindowFlags_Tooltip) {
                ...
                if (window->AutoPosLastDirection == ImGuiDir_None)
                        pos =
                            ref_pos +
                            ImVec2(2,
                                   2);  // If there's not enough room, for
                                        // tooltip we prefer avoiding the cursor
                                        // at all cost even if it means that
...
static bool NavScoreItem(ImGuiNavMoveResult* result, ImRect cand) {
        ...
        if (window->ParentWindow == g.NavWindow) {
                ...
                cand.ClipWithFull(
                    window->ClipRect);  // This allows the scored item to not
                                        // overlap other candidates in the
                                        // parent window
        }
                                  ...
                                  // Press C to rotate.
        {
                ...
                if (quadrant == g.NavMoveDir) {
                        ...
                        ImDrawList* draw_list =
                            ImGui::GetForegroundDrawList(window);
                            ...
                            (nav_bb.Max.y - nav_bb.Min.y) * VISIBLE_RATIO)
                                if (NavScoreItem(
                                        ...
                                        result =
                                            &g.NavMoveResultLocalVisibleSet;
                                        result->ID = id;
                                        result->SelectScopeId =
                                            g.MultiSelectScopeId;
                                        result->Window = window;
                                        result->RectRel = nav_bb_rel;
                                }
        }
                ...
                return ImFloor(ImClamp(
                    ...
                    visible_rect
                        .Max));  // ImFloor() is important because non-integer
                                 // mouse position application in back-end might
...
static void NavScrollToBringItemIntoView(ImGuiWindow* window,
        ...
        ImRect window_rect(window->InnerVisibleRect.Min - ImVec2(1, 1),
                           window->InnerVisibleRect.Max + ImVec2(1, 1));
...
#define NAV_MAP_KEY(_KEY, _NAV_INPUT)                            \
        if (IsKeyDown(g.IO.KeyMap[_KEY])) {                      \
                ...
                if (g.IO.KeyAlt &&
                    !g.IO.KeyCtrl)  // AltGR is Alt+Ctrl, also even on keyboards
                                    // without AltGR we don't want Alt+Ctrl to
                                    // open menu.
...
#undef NAV_MAP_KEY
        ...
        if (g.NavInitResultId != 0 &&
            (!g.NavDisableHighlight || g.NavInitRequestFromMove)) {
                // Apply result from previous navigation init request (will
                // typically select the first item, unless SetItemDefaultFocus()
                // has been called)
                IM_ASSERT(g.NavWindow);
        ...
        if (g.NavMousePosDirty && g.NavIdIsAlive) {
                ...
                if ((g.IO.ConfigFlags &
                    ...
                    (g.IO.BackendFlags & ImGuiBackendFlags_HasSetMousePos)) {
                        if (!g.NavDisableHighlight && g.NavDisableMouseHover &&
                            g.NavWindow) {
                                g.IO.MousePos = g.IO.MousePosPrev =
        ...
        if (IsNavInputPressed(ImGuiNavInput_Cancel,
                ...
                if (g.ActiveId != 0) {
                        if (!(g.ActiveIdBlockNavInputFlags &
                ...
                if (g.NavWindow && !g.NavWindowingTarget && allowed_dir_flags &&
                    !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs)) {
                        ...
                        if ((allowed_dir_flags & (1 << ImGuiDir_Up)) &&
                            IsNavInputPressedAnyOfTwo(
                                ImGuiNavInput_DpadUp, ImGuiNavInput_KeyUp_,
                                ImGuiInputReadMode_Repeat))
        ...
        if (g.NavWindow &&
            ...
            !g.NavWindowingTarget) {
                ...
                const float scroll_speed = ImFloor(
                    window->CalcFontSize() * 100 * g.IO.DeltaTime +
        ...
        // becomes out of bounds, we project its bounding box to the visible
        // area to restart navigation within visible items
        if (g.NavMoveRequest && g.NavMoveFromClampedRefRect &&
            g.NavLayer == 0) {
                ImGuiWindow* window = g.NavWindow;
                ImRect window_rect_rel(
                    window->InnerVisibleRect.Min - window->Pos - ImVec2(1, 1),
                    window->InnerVisibleRect.Max - window->Pos + ImVec2(1, 1));
                        ...
                        window_rect_rel.Expand(ImVec2(
                            ...
                            -ImMin(window_rect_rel.GetHeight(),
                                   pad)));  // Terrible approximation for the
                                            // intent of starting navigation
                                            // from first fully visible item
                        window->NavRectRel[g.NavLayer].ClipWith(
                            window_rect_rel);
                        g.NavId = 0;
                }
        ...
        g.NavScoringRectScreen =
            g.NavWindow ? ImRect(g.NavWindow->Pos + nav_rect_rel.Min,
                                 g.NavWindow->Pos + nav_rect_rel.Max)
                        : GetViewportRect();
        g.NavScoringRectScreen.TranslateY(nav_scoring_rect_offset_y);
        g.NavScoringRectScreen.Min.x = ImMin(
            g.NavScoringRectScreen.Min.x + 1.0f, g.NavScoringRectScreen.Max.x);
        g.NavScoringRectScreen.Max.x = g.NavScoringRectScreen.Min.x;
        IM_ASSERT(!g.NavScoringRectScreen
                       .IsInverted());  // Ensure if we have a finite,
                                        // non-inverted bounding box here will
                                        // allows us to remove extraneous
...
static void ImGui::NavUpdateMoveResult() {
        ...
        if (g.NavLayer == 0) {
                ...
                // Estimate upcoming scroll so we can offset our result position
                // so mouse position can be applied immediately after in
                // NavUpdate()
                ...
                // (we could/should technically recurse back the whole the
                // parent hierarchy).
...
static float ImGui::NavUpdatePageUpPageDown(int allowed_dir_flags) {
        ...
        if (g.NavMoveDir == ImGuiDir_None && g.NavWindow &&
            ...
            !g.NavWindowingTarget && g.NavLayer == 0) {
                ...
                bool page_down_held =
                    IsKeyDown(g.IO.KeyMap[ImGuiKey_PageDown]) &&
                    (allowed_dir_flags & (1 << ImGuiDir_Down));
                if (page_up_held !=
                    page_down_held)  // If either (not both) are pressed
                {
                        if (window->DC.NavLayerActiveMask == 0x00 &&
                            window->DC.NavHasScroll) {
                                // Fallback manual-scroll when window has no
                                // navigable item
                                if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp],
                                                 true))
                                        SetWindowScrollY(
                                            window, window->Scroll.y -
                                                        window->InnerVisibleRect
                                                            .GetHeight());
                                else if (IsKeyPressed(
                                             g.IO.KeyMap[ImGuiKey_PageDown],
                                             true))
                                        SetWindowScrollY(
                                            window, window->Scroll.y +
                                                        window->InnerVisibleRect
                                ...
                                const ImRect& nav_rect_rel =
                                    window->NavRectRel[g.NavLayer];
                                const float page_offset_y = ImMax(
                                    0.0f, window->InnerVisibleRect.GetHeight() -
                                ...
                                if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp],
                                        ...
                                        nav_scoring_rect_offset_y =
                                            +page_offset_y;
                                        g.NavMoveDir =
                                            ImGuiDir_Up;  // Because our scoring
                                                          // rect is offset, we
                                                          // intentionally
                                                          // request the
                                                          // opposite direction
...
static void ImGui::NavUpdateWindowing() {
        ...
        if (g.NavWindowingTarget &&
            g.NavInputSource == ImGuiInputSource_NavGamepad) {
                ...
                if (!IsNavInputDown(ImGuiNavInput_Menu)) {
                        g.NavWindowingToggleLayer &=
                            (g.NavWindowingHighlightAlpha <
                             1.0f);  // Once button was held long enough we
                                     // don't consider it a tap-to-toggle-layer
                                     // press anymore.
        ...
        // back-end clearing releases all keys on ALT-TAB
        if (IsNavInputPressed(ImGuiNavInput_KeyMenu_,
                              ImGuiInputReadMode_Pressed))
                            ...
                            ImMin(g.IO.DisplayFramebufferScale.x,
                                  g.IO.DisplayFramebufferScale
                                      .y));  // FIXME: Doesn't code variable
                                             // framerate very well
                        g.NavWindowingTarget->RootWindow->Pos +=
...
void ImGui::PushColumnsBackground() {
        ...
        PushClipRect(columns->HostClipRect.Min, columns->HostClipRect.Max,
                     false);
        IM_ASSERT(cmd_size == window->DrawList->CmdBuffer
                                  .Size);  // Being in channel 0 this should not
...
void ImGui::BeginColumns(const char* str_id, int columns_count,
        ...
        columns->OffMinX = window->DC.Indent.x -
                           g.Style.ItemSpacing.x;  // Lock our horizontal range
        columns->OffMaxX = ImMax(content_region_width - window->Scroll.x,
        ...
        for (int n = 0; n < columns_count; n++) {
                ...
                float clip_x1 =
                    ImFloor(0.5f + window->Pos.x + GetColumnOffset(n));
                float clip_x2 = ImFloor(0.5f + window->Pos.x +
                                        GetColumnOffset(n + 1) - 1.0f);
...
const ImGuiPayload* ImGui::AcceptDragDropPayload(const char* type,
        ...
        ImGuiPayload& payload = g.DragDropPayload;
        IM_ASSERT(
            g.DragDropActive);  // Not called between BeginDragDropTarget() and
                                // EndDragDropTarget() ?
        ...
        for (char* line = buf; line < buf_end; line = line_end + 1) {
                ...
                if (line[0] == '[' && line_end > line && line_end[-1] == ']') {
                        ...
                        if (!type_end || !name_start) {
                                name_start =
                                    type_start;  // Import legacy entries that
                                                 // have no type
                                type_start = "Window";
                        } else {
...
static void* SettingsHandlerWindow_ReadOpen(ImGuiContext*,
        ...
        ImGuiWindowSettings* settings =
            ImGui::FindWindowSettings(ImHashStr(name));
...
static const char* GetClipboardTextFn_DefaultImpl(void*) {
        ...
        if (ImWchar* wbuf_global = (ImWchar*)::GlobalLock(wbuf_handle)) {
                int buf_len =
                    ImTextCountUtf8BytesFromStr(wbuf_global, NULL) + 1;
...
void ImGui::ShowMetricsWindow(bool* p_open) {
        ...
        struct Funcs {
                static ImRect GetRect(ImGuiWindow* window, int rect_type) {
                        ...
                        } else if (rect_type == RT_InnerVisibleRect) {
                                return window->InnerVisibleRect;
                        } else if (rect_type == RT_InnerWorkRect) {
                                return window->InnerWorkRect;
                        } else if (rect_type == RT_InnerWorkRectClipped) {
                                return window->InnerWorkRectClipped;
                ...
                static void NodeDrawList(ImGuiWindow* window,
                        ...
                        bool node_open = ImGui::TreeNode(
                            draw_list, "%s: '%s' %d vtx, %d indices, %d cmds",
                            label,
                        ...
                        if (draw_list == ImGui::GetWindowDrawList()) {
                                ...
                                ImGui::TextColored(
                                    ...
                                    "CURRENTLY APPENDING");  // Can't display
                                                             // stats for active
                                                             // draw list! (we
                                                             // don't have the
                                                             // data
                        ...
                        ImDrawList* fg_draw_list = GetForegroundDrawList(
                            window);  // Render additional visuals into the
                                      // top-most draw list
                        ...
                        for (const ImDrawCmd* pcmd =
                             ...
                             elem_offset += pcmd->ElemCount, pcmd++) {
                                ...
                                if (show_drawcmd_clip_rects && fg_draw_list &&
                                    ImGui::IsItemHovered()) {
                                        ...
                                        clip_rect.Floor();
                                        fg_draw_list->AddRect(
                                            clip_rect.Min, clip_rect.Max,
                                            IM_COL32(255, 255, 0, 255));
                                        vtxs_rect.Floor();
                                        fg_draw_list->AddRect(
                                            vtxs_rect.Min, vtxs_rect.Max,
                                            IM_COL32(255, 0, 255, 255));
                                }
                                if (!pcmd_node_open) continue;

                                // Display individual triangles/vertices. Hover
                                // on to get the corresponding triangle
                                // highlighted.
                                ImGuiListClipper clipper(
                                    pcmd->ElemCount /
                                    3);  // Manually coarse clip our print out
                                                        ...
                                                        ImDrawListFlags
                                                            backup_flags =
                                                                fg_draw_list
                                                                    ->Flags;
                                                        fg_draw_list->Flags &=
                                                            ~ImDrawListFlags_AntiAliasedLines;  // Disable AA on triangle outlines at is more readable for very large and thin triangles.
                                                        fg_draw_list
                                                            ->AddPolyline(
                                                                triangles_pos,
                                                                3,
                ...
                static void NodeWindows(ImVector<ImGuiWindow*>& windows,
                                        const char* label) {
                        if (!ImGui::TreeNode(label, "%s (%d)", label,
                                             windows.Size))
                                return;
                ...
                static void NodeWindow(ImGuiWindow* window, const char* label) {
                        ...
                        ImGui::BulletText(
                            "Pos: (%.1f,%.1f), Size: (%.1f,%.1f), SizeContents "
                            "(%.1f,%.1f)",
                        ...
                        ImGui::BulletText(
                            ...
                            (flags & ImGuiWindowFlags_AlwaysAutoResize)
                                ...
                                : "");
                        ImGui::BulletText(
                            "Scroll: (%.2f/%.2f,%.2f/%.2f)", window->Scroll.x,
                            GetWindowScrollMaxX(window), window->Scroll.y,
                            GetWindowScrollMaxY(window));
                        ImGui::BulletText(
                            "Active: %d/%d, WriteAccessed: %d, "
                            "BeginOrderWithinContext: %d",
                            ...
                            (window->Active || window->WasActive)
                                ...
                                : -1);
                        ImGui::BulletText(
                            "Appearing: %d, Hidden: %d (CanSkip %d Cannot %d), "
                            "SkipItems: %d",
                ...
                static void NodeTabBar(ImGuiTabBar* tab_bar) {
                        // Standalone tab bars (not associated to
                        // docking/windows functionality) currently hold no
                        // discernible strings.
                        char buf[256];
                        char* p = buf;
                        const char* buf_end = buf + IM_ARRAYSIZE(buf);
                        ImFormatString(p, buf_end - p, "TabBar (%d tabs)%s",
                                       tab_bar->Tabs.Size,
                                       (tab_bar->PrevFrameVisible <
                                        ImGui::GetFrameCount() - 2)
                                           ? " *Inactive*"
                                           : "");
                        if (ImGui::TreeNode(tab_bar, "%s", buf)) {
                                for (int tab_n = 0; tab_n < tab_bar->Tabs.Size;
                                     tab_n++) {
        ...
        // Access private state, we are going to display the draw lists from
        // last frame
        ImGuiContext& g = *GImGui;
        ...
        if (ImGui::TreeNode("Internal state")) {
                ...
                ImGui::Text(
                    ...
                    g.HoveredIdAllowOverlap);  // Data is "in-flight" so
                                               ...
                                               // not
                ImGui::Text(
                    "ActiveId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d, "
                    "Source: %s",
                ...
                ImGui::Text(
                    "DragDrop: %d, SourceId = 0x%08X, Payload \"%s\" (%d "
                    "bytes)",
        ...
        if (ImGui::TreeNode("Tools")) {
                ImGui::Checkbox("Show windows begin order",
                                &show_windows_begin_order);
                ...
                if (show_windows_rects && g.NavWindow) {
                        ImRect r =
                            Funcs::GetRect(g.NavWindow, show_windows_rect_type);
                        ImGui::BulletText(
                            "'%s': (%.1f,%.1f) (%.1f,%.1f) Size (%.1f,%.1f)",
                            g.NavWindow->Name, r.Min.x, r.Min.y, r.Max.x,
                            r.Max.y, r.GetWidth(), r.GetHeight());
                }
                ImGui::Checkbox(
                    "Show clipping rectangle when hovering ImDrawCmd node",
        ...
        if (show_windows_rects || show_windows_begin_order) {
                for (int n = 0; n < g.Windows.Size; n++) {
                        ...
                        if (show_windows_rects) {
                                ImRect r = Funcs::GetRect(
                                    window, show_windows_rect_type);
