...
#define IM_OFFSETOF(_TYPE, _MEMBER) \
        ((size_t) &                 \
         (((_TYPE*)0)->_MEMBER))  // Offset of _MEMBER within _TYPE.
                                  // Standardized as offsetof() in modern C++.
#define IM_UNUSED(_VAR) \
        ((void)         \
             _VAR)  // Used to silence "unused variable warnings". Often useful
...
enum ImGuiWindowFlags_ {
        ...
        ImGuiWindowFlags_HorizontalScrollbar =
            1
            << 11,  // Allow horizontal scrollbar to appear (off by default).
        ...
        ImGuiWindowFlags_NoBringToFrontOnFocus =
            1
            << 13,  // Disable bringing window to front when taking focus (e.g.
        ...
        ImGuiWindowFlags_UnsavedDocument =
            1 << 20,  // Append '*' to title without affecting the ID, as a
                      ...
                      // in a tab/docking context, tab is selected on closure
                      // and closure is deferred by one frame to allow code to
                      // cancel the closure (with a confirmation popup, etc.)
                      // without flicker.
        ...
        ImGuiWindowFlags_NavFlattened =
            1 << 23,  // [BETA] Allow gamepad/keyboard navigation to cross over
                      // parent border to this child (only use on child that
                      // have no scrolling!)
        ...
        // 1 << 17,  // --> Set io.ConfigWindowsResizeFromEdges and make sure
...
enum ImGuiInputTextFlags_ {
        ...
        ImGuiInputTextFlags_CallbackResize =
            1 << 18,  // Callback on buffer capacity changes request (beyond
                      ...
                      // will be provided a new BufSize in the callback and NEED
                      // to honor it. (see misc/cpp/imgui_stdlib.h for an
...
enum ImGuiTreeNodeFlags_ {
        ...
        ImGuiTreeNodeFlags_AllowItemOverlap =
            1 << 2,  // Hit testing to allow subsequent widgets to overlap this
                     // one
        ...
        ImGuiTreeNodeFlags_FramePadding =
            1
            << 10,  // Use FramePadding (even for an unframed text node) to
                    // vertically align text baseline to regular widget height.
                    // Equivalent to calling AlignTextToFramePadding().
        // ImGuiTreeNodeFlags_SpanAllAvailWidth  = 1 << 11,  // FIXME: TODO:
        // Extend hit box horizontally even if not framed
        // ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 12,  // FIXME: TODO:
...
enum ImGuiTabBarFlags_ {
        ...
        ImGuiTabBarFlags_NoCloseWithMiddleMouseButton =
            1 << 3,  // Disable behavior of closing tabs (that are submitted
...
enum ImGuiTabItemFlags_ {
        ...
        ImGuiTabItemFlags_UnsavedDocument =
            1 << 0,  // Append '*' to title without affecting the ID, as a
                     // convenience to avoid using the ### operator. Also: tab
                     // is selected on closure and closure is deferred by one
        ...
        ImGuiTabItemFlags_NoCloseWithMiddleMouseButton =
            1 << 2,  // Disable behavior of closing tabs (that are submitted
                     // with p_open != NULL) with middle mouse button. You can
                     // still repro this behavior on user's side with if
                     // (IsItemHovered() && IsMouseClicked(2)) *p_open = false.
...
enum ImGuiFocusedFlags_ {
        ...
        ImGuiFocusedFlags_AnyWindow =
            1 << 2,  // IsWindowFocused(): Return true if any window is focused.
                     // Important: If you are trying to tell how to dispatch
                     // your low-level inputs, do NOT use this. Use
...
enum ImGuiHoveredFlags_ {
        ...
        ImGuiHoveredFlags_AllowWhenOverlapped =
            1 << 6,  // Return true even if the position is overlapped by
                     // another window
...
enum ImGuiDragDropFlags_ {
        ...
        ImGuiDragDropFlags_SourceNoDisableHover =
            1 << 1,  // By default, when dragging we clear data so that
                     ...
                     // user code submitting tooltips. This flag disable this
        ...
        ImGuiDragDropFlags_SourceExtern =
            1
            << 4,  // External source (from outside of imgui), won't attempt to
                   // read current item/window info. Will always return true.
...
enum ImGuiConfigFlags_ {
        ...
        ImGuiConfigFlags_NavEnableSetMousePos =
            1 << 2,  // Instruct navigation to move the mouse cursor. May be
                     ...
                     // io.WantSetMousePos requests in your binding, otherwise
        ...
        ImGuiConfigFlags_NoMouseCursorChange =
            1 << 5,  // Instruct back-end to not alter mouse cursor shape and
                     // visibility. Use if the back-end cursor changes are
...
enum ImGuiBackendFlags_ {
        ...
        ImGuiBackendFlags_HasGamepad =
            1
            << 0,  // Back-end supports gamepad and currently has one connected.
        ImGuiBackendFlags_HasMouseCursors =
            1 << 1,  // Back-end supports honoring GetMouseCursor() value to
                     // change the OS cursor shape.
        ImGuiBackendFlags_HasSetMousePos =
            1 << 2  // Back-end supports io.WantSetMousePos requests to
...
enum ImGuiColorEditFlags_ {
        ...
        ImGuiColorEditFlags_Float =
            1
            << 24,  // [DataType]   // ColorEdit, ColorPicker, ColorButton:
...
enum ImGuiCond_ {
        ...
        ImGuiCond_Once = 1 << 1,    // Set the variable once per runtime session
                                    // (only the first call with succeed)
...
struct ImGuiStyle {
        ...
        float
            WindowBorderSize;  // Thickness of border around windows. Generally
        ...
        ImVec2 FramePadding;  // Padding within a framed rectangle (used by most
                              // widgets).
        float
            FrameRounding;  // Radius of frame corners rounding. Set to 0.0f to
        ...
        ImVec2 TouchExtraPadding;  // Expand reactive bounding box for
        ...
        ImVec2 SelectableTextAlign;  // Alignment of selectable text when
                                     // selectable is larger than text. Defaults
        ...
        ImVec2
            DisplaySafeAreaPadding;  // If you cannot see the edges of your
                                     ...
                                     // area padding. Apply to popups/tooltips
                                     // as well regular windows. NB: Prefer
...
struct ImGuiIO {
        ...
        ImGuiBackendFlags
            BackendFlags;  // = 0              // See ImGuiBackendFlags_ enum.
                           ...
                           // back-end.
        ImVec2
            DisplaySize;  // <unset>          // Main display size, in pixels.
        ...
        ImFontAtlas* Fonts;  // <auto>           // Load, rasterize and pack one
                             // or more fonts into a single texture.
        float FontGlobalScale;  // = 1.0f           // Global scale all fonts
        bool
            FontAllowUserScaling;  // = false          // Allow user scaling
        ...
        ImVec2 DisplayFramebufferScale;  // = (1, 1)         // For retina
                                         ...
                                         // from framebuffer coordinates. This
        ...
        bool ConfigMacOSXBehaviors;  // = defined(__APPLE__) // OS X style: Text
                                     // editing cursor movement using Alt
        ...
        bool ConfigInputTextCursorBlink;  // = true           // Set to false to
                                          ...
                                          // to 1.63)
        bool
            ConfigWindowsResizeFromEdges;  // = true           // Enable
        ...
        void*
            ImeWindowHandle;  // = NULL           // (Windows) Set this to your
                              // HWND to get automatic IME cursor positioning.

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
        // [OBSOLETE since 1.60+] Rendering function, will be automatically
        // called in Render(). Please call your rendering function yourself now!
        // You can obtain the ImDrawData* by calling ImGui::GetDrawData() after
        // Render(). See example applications if you are unsure of how to
        // implement this.
        ...
        ImVec2 MousePos;    // Mouse position, in pixels. Set to
                            // ImVec2(-FLT_MAX,-FLT_MAX) if mouse is unavailable
                            // (on another screen, etc.)
        bool MouseDown[5];  // Mouse buttons: 0=left, 1=right, 2=middle +
                            // extras. ImGui itself mostly only uses left button
                            // (BeginPopupContext** are using right button).
                            // Others buttons allows us to track if the mouse is
                            // being used by your application + available to
        ...
        float
            NavInputs[ImGuiNavInput_COUNT];  // Gamepad inputs. Cleared back to
        ...
        bool WantCaptureMouse;  // When io.WantCaptureMouse is true, imgui will
                                // use the mouse inputs, do not dispatch them to
                                // your main game/application (in both cases,
                                // always pass on mouse inputs to imgui). (e.g.
        ...
        bool WantCaptureKeyboard;  // When io.WantCaptureKeyboard is true, imgui
                                   // will use the keyboard inputs, do not
                                   // dispatch them to your main
                                   // game/application (in both cases, always
                                   // pass keyboard inputs to imgui). (e.g.
                                   // InputText active, or an imgui window is
                                   // focused and navigation is enabled, etc.).
        bool WantTextInput;  // Mobile/console: when io.WantTextInput is true,
                             // you may display an on-screen keyboard. This is
                             // set by ImGui when it wants textual keyboard
                             // input to happen (e.g. when a InputText widget is
                             // active).
        ...
        bool WantSaveIniSettings;  // When manual .ini load/save is active
                                   ...
                                   // to notify your application that you can
                                   // call SaveIniSettingsToMemory() and save
                                   // yourself. IMPORTANT: You need to clear
                                   // io.WantSaveIniSettings yourself.
        bool NavActive;  // Directional navigation is currently allowed (will
                         // handle ImGuiKey_NavXXX events) = a window is focused
                         // and it doesn't use the ImGuiWindowFlags_NoNavInputs
                         // flag.
        bool NavVisible;  // Directional navigation is visible and allowed (will
                          // handle ImGuiKey_NavXXX events).
        float Framerate;  // Application framerate estimation, in frame per
        ...
        bool MouseDownOwned[5];  // Track if button was clicked inside an imgui
                                 // window. We don't request mouse capture from
...
struct ImGuiInputTextCallbackData {
        ...
        ImWchar EventChar;  // Character input                      //
                            ...
                            // with another one, or set to zero to drop. return
...
struct ImGuiListClipper {
        ...
        // items_count:  Use -1 to ignore (you can call Begin later). Use
        // INT_MAX if you don't know how many items you have (in which case the
        // cursor won't be advanced in the final step). items_height: Use -1.0f
        // to be calculated automatically on first step. Otherwise pass in the
        // distance between your items, typically GetTextLineHeightWithSpacing()
        // or GetFrameHeightWithSpacing(). If you don't specify an items_height,
        // you NEED to call Step(). If you specify items_height you may call the
        // old Begin()/End() api directly, but prefer calling Step().
...
struct ImDrawCmd {
        unsigned int ElemCount;  // Number of indices (multiple of 3) to be
                                 // rendered as triangles. Vertices are stored
        ...
        ImTextureID TextureId;  // User-provided texture ID. Set by user in
                                // ImfontAtlas::SetTexID() for fonts or passed
        ...
        ImDrawCallback UserCallback;  // If != NULL, call the function instead
                                      // of rendering the vertices. clip_rect
...
enum ImDrawListFlags_ {
        ...
        ImDrawListFlags_AntiAliasedLines =
            1
            << 0,  // Lines are anti-aliased (*2 the number of triangles
                   // for 1.0f wide line, otherwise *3 the number of triangles)
        ImDrawListFlags_AntiAliasedFill =
            1 << 1  // Filled shapes have anti-aliased edges (*2 the number of
                    // vertices)
};
...
struct ImDrawList {
        ...
        IMGUI_API void AddRect(
            ...
            float thickness =
                1.0f);  // a: upper-left, b: lower-right (== upper-left + size),
                        // rounding_corners_flags: 4-bits corresponding to which
                        // corner to round
        ...
        IMGUI_API void AddRectFilledMultiColor(const ImVec2& a, const ImVec2& b,
                                               ...
                                               ImU32 col_bot_right,
                                               ImU32 col_bot_left);
        IMGUI_API void AddQuad(const ImVec2& a, const ImVec2& b,
        ...
        IMGUI_API void PathArcTo(const ImVec2& centre, float radius,
...
struct ImDrawData {
        ...
        ImVec2 DisplaySize;  // Size of the viewport to render (==
                             ...
                             // (DisplayPos + DisplaySize == lower-right of the
        ...
        IMGUI_API void ScaleClipRects(
            const ImVec2&
                fb_scale);  // Helper to scale the ClipRect field of each
                            ...
                            // a different scale than ImGui expects, or if there
                            // is a difference between your window resolution
...
struct ImFontAtlas {
        ...
        // Read misc/fonts/README.txt for more details about using colorful
        // icons.
        struct CustomRect {
                unsigned int ID;  // Input    // User ID. Use <0x10000 to map
                                  // into a font glyph, >=0x10000 for
                ...
                ImFont* Font;         // Input    // For custom font glyphs only
                                      // (ID<0x10000): target font
                CustomRect() {
                        ID = 0xFFFFFFFF;
                        Width = Height = 0;
                        X = Y = 0xFFFF;
        ...
        int TexDesiredWidth;  // Texture width desired by user before Build().
                              // Must be a power-of-two. If have many glyphs
                              // your graphics API have texture size
                              // restrictions you may want to increase texture
                              // width to decrease height.
        int TexGlyphPadding;  // Padding between glyphs within texture in
                              // pixels. Defaults to 1. If your rendering method
                              // doesn't rely on bilinear filtering you may set
                              // this to 0.
        ...
        ImVector<CustomRect> CustomRects;   // Rectangles for packing custom
                                            // texture data into the atlas.
        ImVector<ImFontConfig> ConfigData;  // Internal data
        int CustomRectIds[1];  // Identifiers of custom texture rectangle used
                               // by ImFontAtlas/ImDrawList
...
struct ImFont {
        ...
        ImVec2 DisplayOffset;  // 8     // in  // = (0,0)    // Offset font
                               // rendering by xx pixels
        ...
        ImWchar
            FallbackChar;  // 2     // in  // = '?'      // Replacement glyph if
        ...
        float Ascent,
            Descent;  // 4+4   // out //            // Ascent: distance from top
                      // to bottom of e.g. 'A' [0..FontSize]
        ...
        IMGUI_API void AddGlyph(ImWchar c, float x0, float y0, float x1,
        ...
        IMGUI_API void AddRemapChar(
            ...
            bool overwrite_dst =
                true);  // Makes 'dst' character/glyph points to 'src'
                        // character/glyph. Currently needs to be called AFTER
